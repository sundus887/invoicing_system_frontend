...
export default function UserRoles() {
  ...
  // State for role template configuration
  ...
  // Handle role template configuration
  const handleConfigureRole = (roleKey, roleData) => {
    setEditingRole(roleKey);
    setRoleConfigForm({
      name: roleData.label || roleKey,
  ...
  const handleSaveRoleConfig = async (e) => {
    e.preventDefault();
    try {
      // Update role template
      const updatedRoleTemplates = {
        ...roleTemplates,
        [editingRole]: {
          ...roleTemplates[editingRole],
          label: roleConfigForm.name,
          description: roleConfigForm.description,
          permissions: roleConfigForm.permissions
        }
      };
      
      // Save to backend (you can implement this API call)
      // await api.put(`/api/users/meta/role-templates/${editingRole}`, roleConfigForm);
      
      // Update local state
      setRoleTemplates(updatedRoleTemplates);
      ...
      alert('Role template updated successfully!');
    } catch (err) {
      console.error('❌ Error updating role template:', err);
      alert('Failed to update role template. Please try again.');
    }
  };

  const handleCancelRoleConfig = () => {
    setShowRoleConfig(false);
    setEditingRole(null);
    setRoleConfigForm({
      name: "",
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {Object.entries(roleTemplates).map(([roleKey, role], idx) => (
                <div key={idx} className="bg-white rounded-xl p-6 border flex flex-col items-start">
                  ...
                  <button 
                    onClick={() => handleConfigureRole(roleKey, role)}
                    className="flex items-center gap-2 border rounded-lg px-4 py-2 text-sm hover:bg-gray-100"
                  >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">Configure {editingRole} Role</h3>
              <button 
                onClick={handleCancelRoleConfig}
            ...
            <form onSubmit={handleSaveRoleConfig} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={roleConfigForm.name}
                  onChange={(e) => setRoleConfigForm({...roleConfigForm, name: e.target.value})}
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {[
                    ...
                    <label key={permission} className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={roleConfigForm.permissions.includes(permission)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setRoleConfigForm({
                              ...roleConfigForm,
                              permissions: [...roleConfigForm.permissions, permission]
                            });
                          } else {
                            setRoleConfigForm({
                              ...roleConfigForm,
                              permissions: roleConfigForm.permissions.filter(p => p !== permission)
                            });
                          }
                        }}
                        className="rounded"
